...
class VoteWorkGenreAdd(models.Model):
    """
    匿名投票（count集計）
    - medium=NULL の大ジャンル提案も許可
    - is_must_medium=true の major に対する medium=NULL 投票はサービス層で弾く
    """
    ...
    major_genre = models.ForeignKey(
        "MajorGenre",
        on_delete=models.CASCADE,
        related_name="work_add_vote_relations",
    )
    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                # medium_genreがNullの場合、workとmajor_genreの組み合わせにて一意とする。
                fields=["work", "major_genre"],
                condition=Q(medium_genre__isnull=True),
                name="uniq_addvote_work_major_when_medium_null",
            ),
            models.UniqueConstraint(
                # workとmedium_genreの組み合わせにて一意とする。
                fields=["work", "medium_genre"],
                condition=Q(medium_genre__isnull=False),
                name="uniq_addvote_work_medium_when_medium_not_null",
            ),
        ]
        indexes = [
